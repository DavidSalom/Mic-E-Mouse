...
mouse_model_digit = AutoModelForAudioClassification.from_pretrained("facebook/wav2vec2-base", num_labels=10).to(device)
mouse_model_speaker = AutoModelForAudioClassification.from_pretrained("facebook/wav2vec2-base", num_labels=61).to(device)
if not mouse_only:
    wavef_model_digit = AutoModelForAudioClassification.from_pretrained(f"/media/data/models/{run_name}/wavef_model_digit", num_labels=10).to(device)     
    wavef_model_speaker = AutoModelForAudioClassification.from_pretrained(f"/media/data/models/{run_name}/wavef_model_speaker", num_labels=61).to(device)
...
sd = {}
with safe_open(f"/media/data/models/{run_name}/mouse_model_digit/model.safetensors", "pt") as f:
    for k in f.keys():
        sd[k] = f.get_tensor(k)
mouse_model_digit.load_state_dict(sd)

with safe_open(f"/media/data/models/{run_name}/mouse_model_speaker/model.safetensors", "pt") as f:
    for k in f.keys():
        sd[k] = f.get_tensor(k)
mouse_model_speaker.load_state_dict(sd)
del sd
...
# Write accs to file
with open(f"{run_name}", "w") as f:
    f.write(f"MS_DG_ACC: {MS_DG_ACC}\n")
    f.write(f"MS_SP_ACC: {MS_SP_ACC}\n")
    if not mouse_only:
        f.write(f"WV_DG_ACC: {WV_DG_ACC}\n")
        f.write(f"WV_SP_ACC: {WV_SP_ACC}\n")
